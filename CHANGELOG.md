...
- **One Settings dialog per space.** A space's menu now opens a single **Settings** dialog with two tabs. **General** holds the name-and-icon field, **Leave space** for members who hold access through a team, and **Delete space** for admins behind the existing type-the-name confirmation. **Members** is the teams-and-members roster: each row has one quiet menu with **Admin**, **Member** and **Remove**, and **Add people** stays collapsed until you search. Opening members from a note's header lands in the same dialog. Names save on blur, Enter or close, and a draft you never touched can no longer overwrite a teammate's rename. The old inline rename, the separate members dialog and the **Change emoji** item are gone. (#2186)
