...
The subsequent readiness pass ran the complete suite on the pinned Node **24.21.0**: **3,733 passed (89 database tests and 3,644 other tests), 27 skipped, zero failures**. Lint, types, locales, renderer compilation, publication hygiene, and Actionlint passed. The existing two icon lint warnings and renderer chunk-size warning remain.
...
Local verification of the follow-up on Node **24.21.0**: **3,742 tests passed (89 database tests and 3,653 other tests), 27 skipped, zero failures**; lint, types, locales, renderer build, publication hygiene, and Actionlint passed. A separate fresh `npm ci --ignore-scripts` checkout reproduced the missing Electron setup and passed the original suite after the explicit install (89 database tests plus 3,644 other passing tests). The isolated Electron smoke passed all **22 checks**, with zero renderer exceptions and zero OpenWhispr requests. The Linux text monitor compiled and linked as x86-64; its packaged presence and architecture are now checked. The test runner reports its database and remaining test groups separately; totals here include both. Quality CI also installs FFmpeg and isolated X11 test dependencies to avoid silently skipping supported media/native-input coverage. Three additional runtime-probe fixtures verify that timeouts, missing executables, and crashes still fail without retries. After a hosted Mac version probe exceeded 15 seconds (an earlier success took 12.65 seconds), only the Mac llama version check now allows 60 seconds and logs elapsed time. These checks do not exercise a physical microphone, paste target, or signed updater.
