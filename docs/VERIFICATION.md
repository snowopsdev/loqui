...
Local verification of the follow-up on Node **24.21.0**: **3,653 tests passed, 27 skipped, zero failures**; lint, types, locales, renderer build, publication hygiene, and Actionlint passed. A separate fresh `npm ci --ignore-scripts` checkout reproduced the missing Electron setup and passed the original 3,644-test suite after the explicit install. The isolated Electron smoke passed all **22 checks**, with zero renderer exceptions and zero OpenWhispr requests. The Linux text monitor compiled and linked as x86-64; its packaged presence and architecture are now checked. These checks do not exercise a physical microphone, paste target, or signed updater.
