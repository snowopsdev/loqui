...
| ChatGPT subscription                   | OpenAI’s `openai/codex` release metadata at `api.github.com`, release assets at `github.com` and its asset CDN for binary verification, plus the installed Codex app-server’s managed authentication/inference connections |
