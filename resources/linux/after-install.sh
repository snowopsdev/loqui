#!/bin/bash
# Post-install script for Loqui (deb/rpm)
...
CHROME_SANDBOX=$(dpkg -L loqui-snowopsdev 2>/dev/null | grep chrome-sandbox || echo "")
if [ -z "$CHROME_SANDBOX" ]; then
  # Fallback: conventional electron-builder install path
  CHROME_SANDBOX="/opt/Loqui/chrome-sandbox"
fi
