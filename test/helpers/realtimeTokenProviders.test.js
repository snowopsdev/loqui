...
test("corti mints one token and shares it across both streams", async () => {
  ...
  assert.deepEqual(
    await fetchRealtimeTokenForProvider("corti-realtime", cortiDeps, {mode:"byok"}, { streams: 2 }),
    ["corti-1", "corti-1"]
  );
...
test('hosted token acquisition is rejected without contacting any broker',async()=>{
 const {fetchRealtimeTokenForProvider}=await load();
 await assert.rejects(fetchRealtimeTokenForProvider('openai-realtime',deps({postServerToken:()=>assert.fail('broker called')}),{mode:'openwhispr'}),/direct speech provider/);
});
