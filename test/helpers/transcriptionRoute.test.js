...
test("openai and groq route to fixed endpoints with provider-validated models", async () => {
  const openai = await resolve({ cloudTranscriptionProvider: "openai" });
  ...
  for (const legacy of ["gpt-4o-mini-transcribe", "gpt-4o-transcribe", "whisper-1"]) {
    const route = await resolve({
      cloudTranscriptionProvider: "openai",
      cloudTranscriptionModel: legacy,
    });
...
test("unknown, absent, and removed providers never fall through to OpenAI", async () => {
  for (const provider of [undefined, "unknown", "openwhispr", "codex"]) {
    const route = await resolve({ cloudTranscriptionProvider: provider });
    assert.equal(route.transport, "error");
    assert.equal(route.code, "UNSUPPORTED_TRANSCRIPTION_PROVIDER");
  }
});
test("removed hosted and enterprise modes require a new explicit choice", async () => {
  for (const mode of ["openwhispr", "enterprise"]) {
    const route = await resolve({ transcriptionMode: mode, cloudTranscriptionProvider: "openai" });
    assert.equal(route.code, "UNSUPPORTED_TRANSCRIPTION_MODE");
  }
});
test("explicit local mode stays local when a stale flag says otherwise", async () => {
  assert.deepEqual(
    await resolve({
...
test("custom endpoints cannot restore removed hosted transcription", async () => {
  const route = await resolve({
    cloudTranscriptionProvider: "custom",
    cloudTranscriptionBaseUrl: "https://api.openwhispr.com/v1",
  });
